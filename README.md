...
> **現況。** 這是 2023 高科大 NKUST「Python 資料分析實務」第 10 組的繳交網站，2026 年才收成可公開的 Showcase Repository。分析口徑維持當年作業；金鑰、學號與作業草稿留在被忽略的 `original-data/`。示範資料到 2021–2023；要更新請自備 TDX 應用程式金鑰。
...
| **金鑰只在 `.env`** | Showcase Repository 不把 TDX `client_id` 寫進程式或 SQLite。本機沒金鑰也能載入 `demo.json.gz` 看圖。 |
